...
//
		...
		//保留剩余的结果，提取所有的object property
		//计算出需要保留的 module
		//不需要再在ontology上进行消除，而只需要在module上进行消除，
		//新的ontology的性质就是
		//module小于原来的ontology，保留所有原来所有剩下
		//保证结果其结果在剩余的concept和role上保留所有的逻辑结果，
		//体现从原始ontology中提取一个和module和
		//module的过程：纯粹原来ontology的子集，module的结果一定能entail ontology
		//forgetting的意义是除了进行保留，还要推理进行遗忘。
		//module是没有涉及推理的工作，推理的计算复杂度

		//在很多状况下，module的结果是小于原始的ontology的。
			//之后的forgetting的结果是
		//Ontology在一定的axiom的数量之上会更好。
		//贡献1：新算法的rule能够处理更多的。。。成功率会增高。
		//贡献2：module在什么时候加入更好。
		...
		if (!c_sig.isEmpty()) {
			...
			System.out.println("c_module = " + c_module.getLogicalAxiomCount());
		...
		if (!di.definer_set.isEmpty()) {
			//返回了所有有definer的axioms，
			//删除了caluse_list_normalised
			//
			...
			System.out.println("Forgetting Successful D3!");
			manager.addAxioms(result, bc.toOWLAxioms(bc.toAxioms(clause_list_normalised)));
			return result;
		}
